...
for j in range(len(json_files)):
    dis2.append([0, 0])
    dis2[j][0] = j
    dis2[j][1] = Distance.DisTwoPoints(neck_coordinates[j][0], neck_coordinates[j][1], wrist_coordinates[j][0], wrist_coordinates[j][1])
...
if dominant == 'right':
    lmax_xx = judge.distinction(lmax_x, neck_coordinates, wrist_coordinates)
if dominant == 'left':
    lmax_xx = judge.distinction(lmax_x, wrist_coordinates, neck_coordinates)
